...
<head>
    <meta charset="UTF-8">
    <title>{{ $savedSchedule->nombre_horario }} - Impresion</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 20px; color: #1f2937; }
        h1 { font-size: 20px; margin-bottom: 4px; }
        p.subtitle { font-size: 13px; color: #6b7280; margin-bottom: 16px; }
...
<body>
    ...
    <h1>{{ $savedSchedule->nombre_horario }}</h1>
    <p class="subtitle">Gestion: {{ $savedSchedule->gestion }}</p>
                    ...
                    @foreach ($dias as $diaNum => $diaNombre)
                        <td>
                            @if (isset($grid[$diaNum][$hora]))
                                ...
                                <strong>{{ $class->subject->sigla }}</strong><br>
